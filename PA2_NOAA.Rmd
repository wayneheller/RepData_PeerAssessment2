...
```{r}
dfMaxPropDamage <- dfEventDamage %>% group_by(YEAR) %>% filter( Total_PropertyDamage== max(Total_PropertyDamage)) %>% arrange(desc(YEAR),EVTYPE,Total_PropertyDamage)

kable(dfMaxPropDamage[, c(1, 2, 3)], digits=2)
```

### Select the event type with the most Crop damage for a specific year
```{r}
dfMaxCropDamage <- dfEventDamage %>% group_by(YEAR) %>% filter( Total_CropDamage== max(Total_CropDamage)) %>% arrange(desc(YEAR),EVTYPE,Total_CropDamage)

kable(dfMaxCropDamage[, c(1, 2, 4)], digits = 2)
